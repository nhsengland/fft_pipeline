...
def main():
    ...
    # Generate list of relevant Excel files from specified folder to enable identification of current period
        ...
        logging.info("Most recent Inpatient FFT file for use as current month within the folder:")
        logging.info( current_month_file)
        logging.info("Most recent Inpatient FFT file for use as previous month within the folder:")
    ...
    #   - aggregation to Integrated Care Board (ICB) level as this is required but doesn't exist in the Extract file
        ...
        current_org_df = load_excel_sheet(current_month_file, 'Parent_Self_Trusts_Organisa')  
        logging.info("Most recent months Organisation Level Inpatient DataFrame after import:")
        ...
        logging.info("Specified columns validated for data length in Organisation Level Inpatient DataFrame.")
        
        # Validate numeric fields all contain values of correct type (integer (int) or decimal (float))
        current_org_df = validate_numeric_columns(current_org_df, ['1 Very Good', '2 Good', '3 Neither good nor poor', 
                                                                  '4 Poor', '5 Very poor', '6 Dont Know', 'Total Responses', 'Total Eligible'], 'int')
        current_org_df = validate_numeric_columns(current_org_df, ['Prop Pos', 'Prop Neg'], 'float')
        logging.info("Specified columns validated for data type in Organisation Level Inpatient DataFrame.")

        # Using current_org_df, get contents of the periodname and yearnumber columns to use for generating the FFT_Period
        current_periodname = get_cell_content_as_string(current_org_df, source_row=0, source_col='Periodname')
        current_yearnumber = get_cell_content_as_string(current_org_df, source_row=0, source_col='Yearnumber')
        logging.info(f"DataFrame period is {current_periodname} and year is {current_yearnumber}")
        ...
        logging.info("Current months Organisation Level Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_org_df.head())   
        ...
        logging.info("Current months Organisation Level Inpatient DataFrame with columns renamed:")
        logging.info(current_org_df.head())
    ...
    # for addition to ICB, Trust, Site and Ward tabs to populate respective header rows
        ...
        current_total_df = remove_columns(current_org_df, ['Trust Code', 'Trust Name', 'ICB Name', 'Percentage Positive', 'Percentage Negative'])
        logging.info("Current months Total Inpatient DataFrame with unwanted columns for Totals removed:")
        ...
        current_total_df = replace_non_matching_values(current_total_df, 'ICB Code', 'IS1', 'NHS')
        logging.info("Current months Total Inpatient DataFrame with NHS ICB Codes replaced with 'NHS':")
        logging.info(current_total_df.head())
        ...
        logging.info("Current months Total Inpatient DataFrame with ICB Code column renamed:")
        logging.info(current_total_df.head())

        # Group by specified columns and sum the numeric columns for NHS/IS1 aggregation
        current_total_df = sum_grouped_response_fields(current_total_df, ['Submitter Type']) 
        ...
        sum_cols_for_totals_agg = ['Very Good', 'Good', 'Neither Good nor Poor', 'Dont Know', 'Poor', 'Very Poor', 'Total Responses', 'Total Eligible']
        # Create a dataframe containing sum total values of NHS/IS1 combined for the period
        sum_current_total_df = create_data_totals(current_total_df, current_fft_period, 'Submitter Type', sum_cols_for_totals_agg)
        ...
        logging.info("NHS/IS1 aggregated current months values appended to Totals DataFrame:")
        logging.info(current_total_df.head())

        # Calculate 'Percentage Positive' for Totals DataFrame
        current_total_df = create_percentage_field(current_total_df, 'Percentage Positive', 'Very Good', 'Good', 'Total Responses')
        logging.info("Current months Totals DataFrame with Percentage Positive field added:")
        logging.info(current_total_df.head())
        
        # Calculate 'Percentage Negative' for Totals DataFrame
        current_total_df = create_percentage_field(current_total_df, 'Percentage Negative', 'Very Poor', 'Poor', 'Total Responses')
        logging.info("Current months Totals DataFrame with Percentage Negative field added:")
        logging.info(current_total_df.head())
        
        # Remove independent provider row to retain only NHS and Total rows
        current_total_df = remove_rows_by_cell_content(current_total_df, 'Submitter Type', 'IS1')
        ...
        totals_output_column_order = ['Total Responses', 'Total Eligible', 'Percentage Positive', 'Percentage Negative', 
        ...
        fields_to_convert_to_object_type = ['Total Responses', 'Total Eligible', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_total_df = convert_fields_to_object_type(current_total_df, fields_to_convert_to_object_type)
        # Clean DataFrame columns replacing with 'NA'
        current_total_df = replace_missing_values(current_total_df, 'NA')
        logging.info("Current months Totals DataFrame following column clean, removal and reorder:")
        logging.info(current_total_df.head()) 
    ...
    # in the final output
        ...
        current_sum_df = remove_columns(current_org_df, ['Trust Code', 'Trust Name', 'ICB Name', 'Total Eligible', 
                                                         'Neither Good nor Poor', 'Dont Know','Percentage Positive', 'Percentage Negative'])
        logging.info("Current months Summary Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_sum_df.head())    

        # Standardise values in ICB Code replacing all ICB Codes with NHS and retaining IS1 for independent providers
        current_sum_df = replace_non_matching_values(current_sum_df, 'ICB Code', 'IS1', 'NHS')
        logging.info("Current months Summary Inpatient DataFrame with NHS ICB Codes replaced with 'NHS':")
        logging.info(current_sum_df.head())
        ...
        logging.info("Current months Summary Inpatient DataFrame with ICB Code column renamed:")
        logging.info(current_sum_df.head())
        
        # Sum number of rows containing NHS and number containing IS1 and store for addition to DataFrame
        sum_level_counts = count_nhs_is1_totals(current_sum_df, 'Submitter Type', 'summary_count_of_IS1', 'summary_count_of_NHS')
        logging.info("Summary level counts of the number of NHS and Independent Provider (IS1) submitters for the current month:")
        ...
        logging.info("NHS/IS1 aggregated current months summary Inpatient DataFrame with new column added:")
        logging.info(current_sum_df.head())        
        
        # Add values to 'Number of organisations submitting' column for independent provider (IS1) and NHS total submitters
        current_sum_df = add_submission_counts_to_df(current_sum_df, 'Submitter Type', sum_level_counts['summary_count_of_IS1'], 
                                                     sum_level_counts['summary_count_of_NHS'], 'Number of organisations submitting')
        logging.info("Current months Summary level Inpatient DataFrame with 'Number of organisations submitting' column added:")
        logging.info(current_sum_df.head())        

        # Specify the columns the current_sum_df to sum for the below function
        sum_cols_to_aggregate = ['Very Good', 'Good', 'Poor', 'Very Poor', 'Total Responses', 'Number of organisations submitting']
        # Create a dataframe containing total values for NHS/IS1 values combined for the period
        sum_total_df = create_data_totals(current_sum_df, current_fft_period, 'Submitter Type', sum_cols_to_aggregate)
        ...
        logging.info("NHS/IS1 aggregated current months Summary level Inpatient DataFrame with Totals appended:")
        logging.info(current_sum_df.head())

        # Calculate 'Percentage Positive' for the summary level DataFrame
        current_sum_df = create_percentage_field(current_sum_df, 'Percentage Positive', 'Very Good', 'Good', 'Total Responses')
        logging.info("Current months Summary level Inpatient DataFrame with Percentage Positive field added:")
        logging.info(current_sum_df.head())
        
        # Calculate 'Percentage Negative' for the  summary level DataFrame
        current_sum_df = create_percentage_field(current_sum_df, 'Percentage Negative', 'Very Poor', 'Poor', 'Total Responses')
        logging.info("Current months Summary level Inpatient DataFrame with Percentage Negative field added:")
        logging.info(current_sum_df.head())
        ...
        logging.info("Current months Summary level Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_sum_df.head())    
    ...
    # Import Monthly Rolling Totals for adding current months totals to, extracting previous months totals, and calculating cumulative values. 
        monthly_rolling_totals = rf'rolling_total_file\Monthly Rolling Totals.xlsx'
        ...
        updated_monthly_rolling_totals = update_monthly_rolling_totals(current_sum_df, ip_rolling_df, current_fft_period)
        ...
        updated_monthly_rolling_totals = update_cumulative_value(updated_monthly_rolling_totals, 
                                                                 'Monthly total responses', 'Total responses to date')
        updated_monthly_rolling_totals = update_cumulative_value(updated_monthly_rolling_totals, 
                                                                 'Monthly NHS responses', 'Total NHS responses to date')
        updated_monthly_rolling_totals = update_cumulative_value(updated_monthly_rolling_totals, 
                                                                 'Monthly independent responses', 'Total independent responses to date')
        logging.info("Monthly Rolling Totals updated with current months cumulative responses to date value:")
        ...
        current_month_row = updated_monthly_rolling_totals.index[-1] # last row of dataframe        
        previous_month_row = updated_monthly_rolling_totals.index[-2] # second to last row of dataframe

        # using updated_monthly_rolling_totals once new data saved, get the previous_fft_period month-year for onward use as prefix
        previous_fft_period = get_cell_content_as_string(updated_monthly_rolling_totals, source_row=previous_month_row, source_col='FFT Period')
        ...
        current_sum_df= add_dataframe_column(current_sum_df, ['Total Responses to Date', 'Previous Months Responses', 
                                                              'Previous Months Percentage Positive', 'Previous Months Percentage Negative'], [0, 0, 0.1, 0.1])
        ...
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Total responses to date', current_month_row, 'Total Responses to Date', 2)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Total NHS responses to date', current_month_row, 'Total Responses to Date', 1)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Total independent responses to date', current_month_row, 'Total Responses to Date', 0)
        # Get previous months responses for Total, IS1 and NHS from Rolling Monthly Totals and add to current_sum_df
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly total responses', previous_month_row, 'Previous Months Responses', 2)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly NHS responses', previous_month_row, 'Previous Months Responses', 1)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly independent responses', previous_month_row, 'Previous Months Responses', 0)
        ...
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly total percentage positive', previous_month_row, 'Previous Months Percentage Positive', 2)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly NHS percentage positive', previous_month_row, 'Previous Months Percentage Positive', 1)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly independent percentage positive', previous_month_row, 'Previous Months Percentage Positive', 0)
        ...
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly total percentage negative', previous_month_row, 'Previous Months Percentage Negative', 2)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly NHS percentage negative', previous_month_row, 'Previous Months Percentage Negative', 1)
        current_sum_df= copy_value_between_dataframes(updated_monthly_rolling_totals, current_sum_df, 
                                                      'Monthly independent percentage negative', previous_month_row, 'Previous Months Percentage Negative', 0)
        ...
        new_column_order = ['Submitter Type', 'Number of organisations submitting', 'Total Responses to Date', 'Total Responses', 
                            'Previous Months Responses', 'Percentage Positive', 'Previous Months Percentage Positive', 
                            'Percentage Negative', 'Previous Months Percentage Negative']
        ...
        logging.info("Summary table with column order changed to conform with expected output:")
        logging.info(current_sum_df.head()) 
        ...
        current_number_orgs_submitting = new_column_name_with_period_prefix(current_fft_period, 'Number of organisations submitting')
        current_total_responses = new_column_name_with_period_prefix(current_fft_period, 'Responses')
        previous_total_responses = new_column_name_with_period_prefix(previous_fft_period, 'Responses')
        current_percent_pos = new_column_name_with_period_prefix(current_fft_period, 'Percentage Positive')
        previous_percent_pos = new_column_name_with_period_prefix(previous_fft_period, 'Percentage Positive')
        current_percent_neg = new_column_name_with_period_prefix(current_fft_period, 'Percentage Negative')
        previous_percent_neg = new_column_name_with_period_prefix(previous_fft_period, 'Percentage Negative')
        ...
        logging.info("Current months Summary Inpatient DataFrame with columns sorted in descending order by the FFT column:")
        logging.info(current_sum_df.head())
    ...
    # Create ICB level DataFrame through aggregation of necessary fields from the Organisation level DataFrame
        ...
        current_icb_df = remove_columns(current_org_df, ['Trust Code', 'Trust Name', 'Percentage Positive', 'Percentage Negative'])        
        logging.info("Current months ICB Level Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_icb_df.head())
        
        # Group by the specified columns and sum the numeric columns for ICB-level aggregation
        current_icb_df = sum_grouped_response_fields(current_icb_df, ['ICB Code', 'ICB Name']) 
        ...
        current_icb_df = create_percentage_field(current_icb_df, 'Percentage Positive', 'Very Good', 'Good', 'Total Responses')
        logging.info("Current months ICB Level Inpatient DataFrame with Percentage Positive field added:")
        logging.info(current_icb_df.head())
        
        # Calculate 'Percentage Positive' for the ICB-level DataFrame
        current_icb_df = create_percentage_field(current_icb_df, 'Percentage Negative', 'Very Poor', 'Poor', 'Total Responses')
        logging.info("Current months ICB Level Inpatient DataFrame with Percentage Negative field added:")
        logging.info(current_icb_df.head())
        ...
        current_icb_df = create_first_level_suppression(current_icb_df, 'first_level_suppression', 'Total Responses')
        logging.info("Current months ICB Level Inpatient DataFrame with first_level_suppression column:")
        logging.info(current_icb_df.head())
        ...
        current_icb_df = create_icb_second_level_suppression(current_icb_df, 'first_level_suppression', 'second_level_suppression')
        logging.info("Current months ICB Level Inpatient DataFrame with second_level_suppression column:")
        logging.info(current_icb_df.head())            
        
        # Add field to show all ICB level rows that require suppression as a result of either first or second level suppression
        current_icb_df = confirm_row_level_suppression(current_icb_df, 'icb_level_suppression_required', 'first_level_suppression','second_level_suppression')
        logging.info("Current months ICB Level Inpatient DataFrame with icb_level_suppression_required column:")
        logging.info(current_icb_df.head())
        ...
        fields_for_converting = ['Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_icb_df = convert_fields_to_object_type(current_icb_df, fields_for_converting)
        logging.info("Current months ICB Level Inpatient DataFrame with columns impacted by suppression converted to string:")
        logging.info(current_icb_df.head())
        ...
        current_icb_df = suppress_data(current_icb_df, 'icb_level_suppression_required', 'first_level_suppression')
        logging.info("Current months ICB Level Inpatient DataFrame with suppression applied:")
        logging.info(current_icb_df.head())      
        
        # Reorder current ICB level DataFrame by ICB Code
        current_icb_df = sort_dataframe(current_icb_df, ['ICB Code'], True)
        logging.info("Current months ICB Level Inpatient DataFrame after sort:")
        logging.info(current_icb_df.head())           

        # Move responses for Independent Service Providers to bottom of DataFrame aligned with Stakeholder requirements for output
        current_icb_df = move_independent_provider_rows_to_bottom(current_icb_df)
        logging.info("Current months ICB Level Inpatient DataFrame with IS1 orgs moved to bottom:")
        logging.info(current_icb_df.tail())  


    # Processes carried out to generate Organisation (with joined collection mode), Site and Ward level data with necessary suppressions applied. 
    # Processes/Transformations include:
    # - using already partially transformed Organisation/Trust level DataFrame (current_org_df) and output from ICB transformation, reorder so all 
    # org/trusts in an ICB are together with Total Responses in ascending order (lowest to highest)
    # - adjust format of percentage positive/negative fields to ensure they are in the correct unit when loaded to output Excel file
    # - add ranking field to show order of lowest to highest number of responses by Organisation within each ICB. Any row with Total Responses = 0, 
    # are 0 and lowest non 0 value ranked as 1
    # - create a new field and implement first level suppression. 
    # - create a new field and implement second level suppression. Where an Organisation within an ICB is the first to be suppressed, and there is 1 
    # or more others in the ICB that have Total Responses above 0, the next lowest Total Responses to the one suppressed will also be suppressed. If 
    # the first e.g., 3 require suppression and a fourth exists (non 0) it too will be suppressed
    # - create a new field for showing whether suppression is required at current level as a result of suppression at upper level. If an ICB required 
    # suppression, the two organisations in the ICB reporting the lowest Total (Non 0) Responses will require suppression regardless of whether they 
    # have already been flagged for first/second level suppression
    # - create a new field confirming row level suppression requiring where either first, second or upper level suppression columns show as True
    # -  convert all fields from numeric to object type that would require suppression being applied (Likert and percentage fields)
    # - apply suppression according to suppression rules – all Likert responses are suppressed with ‘*’ for any row requiring suppression (as per 
    # confirmation column), but where it requires first level suppression, the Percentage fields are suppressed as well. 
    # - sort DataFrame by ICB Code (ascending) and Total Responses (descending) and then split the DataFrame (NHS and Independent Providers) and put 
    # all Independent Provider rows below NHS rows. This required Tests and Function creation.
    # - extract Trust Collection Modes data to a new DataFrame and join these by Site Code to the Organisation/Trust level DataFrame
    # - use a copy to generate aggregated sum totals including/excluding IPs by replacing all non IP ICB Codes with NHS, and create another copy to 
    # generate grand totals and append Totals to NHS/IP totals removing IP totals to retain just grand totals (NHS including IP) and just NHS (excluding Ip) totals
    # - extract site level raw data to a DataFrame and repeat all stages carried out at Trust/Organisation level for suppression, excluding adding 
    # collection mode detail. Ordering for suppression requires grouping all Sites within Organisations within ICB are grouped together and ordered 
    # by Total Responses in ascending order 
    # - following site level suppression reorder DataFrame by ICB Code (ascending), Trust Code (ascending) and Total Responses (descending) and move 
    # all independent provider rows to the bottom
    # - extract ward level raw data to a DataFrame and repeat all stages carried out at site level for suppression. Ordering for suppression requires 
    # grouping all Wards within Sites within Organisations within ICB are grouped together and ordered by Total Responses in ascending order 
    # - following ward level suppression reorder DataFrame by ICB Code (ascending), Trust Code (ascending) Site Code (ascending) and Total Responses 
    # (descending) and move all independent provider rows to the bottom
    # - ward names are manually created by submitters and can contain special characters that may cause processing errors. Using Regular Expression, 
    ...
    # Generate Organisation (Trust) level output using partially cleaned Organisation level DataFrame (current_org_df) and output from ICB transformation 
        ...
        current_org_df = sort_dataframe(current_org_df, ['ICB Code', 'Total Responses'], [True, True])
        ...
        current_org_df = rank_organisation_results(current_org_df, 'ICB Code', 'Total Responses', 'Rank')
        logging.info("Current months Organisation Level Inpatient DataFrame with rank column:")
        logging.info(current_org_df.head())
        ...
        current_org_df = create_first_level_suppression(current_org_df, 'first_level_suppression', 'Total Responses')
        logging.info("Current months Organisation Level Inpatient DataFrame with first_level_suppression column:")
        logging.info(current_org_df.head())
        ...
        current_org_df= create_second_level_suppression(current_org_df, 'first_level_suppression', 'Rank', 'second_level_suppression')
        logging.info("Current months Organisation Level Inpatient DataFrame with second_level_suppression column:")
        logging.info(current_org_df.head())    

        # Add field showing whether suppression is required at current level as a result of suppression at upper level
        current_org_df = add_suppression_required_from_upper_level_column(current_icb_df, current_org_df, 
                                                                          'upper_level_suppression_required', 'ICB Code', 'icb_level_suppression_required')    
        logging.info("Current months Organisation Level Inpatient DataFrame with upper_level_suppression_required column:")
        logging.info(current_org_df.head())
        ...
        current_org_df = confirm_row_level_suppression(current_org_df, 'trust_level_suppression_required', 'first_level_suppression',
                                                       'second_level_suppression', 'upper_level_suppression_required')
        logging.info("Current months Organisation Level Inpatient DataFrame with trudt_level_suppression_required column:")
        logging.info(current_org_df.head())

        # Convert all fields that can contain suppression values from numeric (integer) to string to avoid a data type mismatch error when suppression is applied
        current_org_df = convert_fields_to_object_type(current_org_df, fields_for_converting)
        logging.info("Current months Organisation Level Inpatient DataFrame with columns impacted by suppression converted to string:")
        logging.info(current_org_df.head())
        ...
        current_org_df = suppress_data(current_org_df, 'trust_level_suppression_required', 'first_level_suppression')
        logging.info("Current months Organisation Level Inpatient DataFrame with suppression applied:")
        logging.info(current_org_df.head())      
        
        # Reorder current Organisation level DataFrame by ICB Code and Total Responses
        current_org_df = sort_dataframe(current_org_df, ['ICB Code', 'Total Responses'], [True, False])
        logging.info("Current months Organisation Level Inpatient DataFrame after sort:")
        logging.info(current_org_df.head())           

        # Move responses for Independent Service Providers to bottom of DataFrame aligned with Stakeholder requirements for output
        current_org_df = move_independent_provider_rows_to_bottom(current_org_df)
        logging.info("Current months Organisation Level Inpatient DataFrame with IS1 orgs moved to bottom:")
    ...
    # Retrieve Trust collection modes and join to current_org_df to meet Output requirement
        ...
        current_mode_df = load_excel_sheet(current_month_file, 'Parent_Self_Trusts_Collecti')  
        logging.info("Most recent months Collection Mode Inpatient DataFrame after import:")
        ...
        current_mode_totals_df = remove_columns(current_mode_df, ['Period', 'Yearnumber', 'Periodname', 'Org code', 'Org name', 'STP Name'])
        logging.info("Current months Collection Mode Inpatient DataFrame with unwanted columns removed to generate Totals:")
        logging.info(current_mode_totals_df.head()) 
        
        # Remove Period related columns from source DataFrame as not required for data processing or final product
        current_mode_df = remove_columns(current_mode_df, ['Period', 'Yearnumber', 'Periodname', 'Org name', 'STP Code', 'STP Name'])        
        logging.info("Current months Collection Mode Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_mode_df.head())           
        ...
        logging.info("Current months Collection Mode Inpatient DataFrame with columns renamed:")
        logging.info(current_mode_df.head())

        current_org_df = join_dataframes(current_org_df, current_mode_df, 'Trust Code', 'left', 'one_to_one')
        logging.info("Current months Trust Level Inpatient DataFrame with Collection Mode columns joined:")
        logging.info(current_mode_df.head())
    ...
    # Generate Mode Totals to populate totals fields of the Trust tab
        ...
        current_mode_totals_df = replace_non_matching_values(current_mode_totals_df, 'STP Code', 'IS1', 'NHS')
        logging.info("Current months Total mode Inpatient DataFrame with NHS STP(ICB) Codes replaced with 'NHS':")
        logging.info(current_mode_totals_df.head())
        ...
        current_mode_totals_df = rename_columns(current_mode_totals_df, total_mode_columns_to_rename)        
        logging.info("Current months Total mode Inpatient DataFrame with STP(ICB) Code column renamed:")
        logging.info(current_mode_totals_df.head())

        # Group by specified columns and sum the numeric columns for NHS/IS1 aggregation
        current_mode_totals_df = sum_grouped_response_fields(current_mode_totals_df, ['Submitter Type']) 
        logging.info("NHS/IS1 aggregated current months Total mode values Inpatient DataFrame:")
        logging.info(current_mode_totals_df.head())        

        # Specify the columns in the current_mode_totals_df to sum for the below function to generate Totals
        sum_mode_cols_for_totals_agg = ['Mode SMS', 'Mode Electronic Discharge', 'Mode Electronic Home', 'Mode Paper Discharge', 
                                   'Mode Paper Home', 'Mode Telephone', 'Mode Online', 'Mode Other']
        # Create a dataframe containing sum total values of NHS/IS1 combined for the period
        sum_current_mode_total_df = create_data_totals(current_mode_totals_df, current_fft_period, 'Submitter Type', sum_mode_cols_for_totals_agg)
        ...
        # Append the totals DataFrame to the bottom of the current summary DataFrame containing NHS/IS1 totals
        current_mode_totals_df = append_dataframes(sum_current_mode_total_df, current_mode_totals_df)
        logging.info("NHS/IS1 aggregated current months mode values appended to mode Totals DataFrame:")
        logging.info(current_mode_totals_df.head())
      
        # Remove independent provider row to retain only NHS and Total rows
        current_mode_totals_df = remove_rows_by_cell_content(current_mode_totals_df, 'Submitter Type', 'IS1')
        ...
        current_mode_totals_df = remove_columns(current_mode_totals_df, ['Submitter Type'])

        # Convert numeric fields that may contain missing/null values to object type to enable filling with 'NA' without raising Error warning
        mode_fields_to_convert_to_object_type = ['Mode SMS', 'Mode Electronic Discharge', 'Mode Electronic Home', 'Mode Paper Discharge', 
                                                 'Mode Paper Home', 'Mode Telephone', 'Mode Online', 'Mode Other']
        current_mode_totals_df = convert_fields_to_object_type(current_mode_totals_df, mode_fields_to_convert_to_object_type)
       
        # Clean DataFrame columns replacing with 'NA'
        current_mode_totals_df = replace_missing_values(current_mode_totals_df, 'NA')
        logging.info("Current months mode Totals DataFrame following column clean and removal:")
        logging.info(current_mode_totals_df.head()) 
    ...
    # Generate and process Site Level DataFrame with Upper Level Suppression input from Organisation Level process
        ...
        current_site_df = load_excel_sheet(current_month_file, 'Parent_Self_Trusts_Site_Lev')  
        logging.info("Most recent months Site Level Inpatient DataFrame after import:")
        logging.info(current_site_df.head())

        # Remove Period related columns from source DataFrame as not required for data processing or final product
        current_site_df = remove_columns(current_site_df, ['Period', 'Yearnumber', 'Periodname'])        
        logging.info("Current months Site Level Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_site_df.head())   
        ...
        logging.info("Current months Site Level Inpatient DataFrame with columns renamed:")
        logging.info(current_site_df.head())
        ...
        current_site_df = sort_dataframe(current_site_df, ['ICB Code', 'Trust Code', 'Total Responses'], [True, True, True])
        ...
        current_site_df = rank_organisation_results(current_site_df, 'Trust Code', 'Total Responses', 'Rank')
        ...
        current_site_df = create_first_level_suppression(current_site_df, 'first_level_suppression', 'Total Responses')
        logging.info("Current months Site Level Inpatient DataFrame with first_level_suppression column:")
        logging.info(current_site_df.head())
        ...
        current_site_df= create_second_level_suppression(current_site_df, 'first_level_suppression', 'Rank', 'second_level_suppression')
        logging.info("Current months Site Level Inpatient DataFrame with second_level_suppression column:")
        logging.info(current_site_df.head())    

        # Add field showing whether suppression is required at current level as a result of suppression at upper level
        current_site_df = add_suppression_required_from_upper_level_column(current_org_df, current_site_df, 
                                                                          'upper_level_suppression_required', 'Trust Code', 'trust_level_suppression_required')    
        logging.info("Current months Site Level Inpatient DataFrame with upper_level_suppression_required column:")
        logging.info(current_site_df.head())
        ...
        current_site_df = confirm_row_level_suppression(current_site_df, 'site_level_suppression_required', 'first_level_suppression',
                                                       'second_level_suppression', 'upper_level_suppression_required')
        logging.info("Current months Site Level Inpatient DataFrame with site_level_suppression_required column:")
        logging.info(current_site_df.head())

        # Convert all fields that can contain suppression values from numeric (integer) to string to avoid a data type mismatch error when suppression is applied
        current_site_df = convert_fields_to_object_type(current_site_df, fields_for_converting)
        logging.info("Current months Site Level Inpatient DataFrame with columns impacted by suppression converted to string:")
        logging.info(current_site_df.head())
        ...
        current_site_df = suppress_data(current_site_df, 'site_level_suppression_required', 'first_level_suppression')
        logging.info("Current months Site Level Inpatient DataFrame with suppression applied:")
        logging.info(current_site_df.head())      
        
        # Reorder current Site level DataFrame by ICB Code, Turst Code and Total Responses (highest to lowest)
        current_site_df = sort_dataframe(current_site_df, ['ICB Code', 'Trust Code', 'Total Responses'], [True, True, False])
        logging.info("Current months Site Level Inpatient DataFrame after sort:")
        logging.info(current_site_df.head())           

        # Move responses for Independent Service Providers to bottom of DataFrame aligned with Stakeholder requirements for output
        current_site_df = move_independent_provider_rows_to_bottom(current_site_df)
        logging.info("Current months Site Level Inpatient DataFrame with IS1 orgs moved to bottom:")
    ...
    # Generate and process current Ward level DataFrame with Upper Level Suppression input from Site Level process
        ...
        current_ward_df = load_excel_sheet(current_month_file, 'Parent_Self_Trusts_Ward_Lev')  
        logging.info("Most recent months Ward Level Inpatient DataFrame after import:")
        logging.info(current_ward_df.head())

        # Remove Period related columns from source DataFrame as not required for data processing or final product
        current_ward_df = remove_columns(current_ward_df, ['Period', 'Yearnumber', 'Periodname'])        
        logging.info("Current months Ward Level Inpatient DataFrame with unwanted columns removed:")
        logging.info(current_ward_df.head())   
    ...
    #        'FFT_Period': 'Period', 
        ...
        logging.info("Current months Ward Level Inpatient DataFrame with columns renamed:")
        logging.info(current_ward_df.head())
        ...
        # are grouped together and ordered by total responses (lowest to highest)
        current_ward_df = sort_dataframe(current_ward_df, ['ICB Code', 'Trust Code', 'Site Code', 'Total Responses'], [True, True, True, True])
        ...
        current_ward_df = rank_organisation_results(current_ward_df, 'Trust Code', 'Total Responses', 'Rank')
        ...
        # will be flagged as requiring suppression.
        current_ward_df = create_first_level_suppression(current_ward_df, 'first_level_suppression', 'Total Responses')
        logging.info("Current months Ward Level Inpatient DataFrame with first_level_suppression column:")
        logging.info(current_ward_df.head())
        ...
        current_ward_df= create_second_level_suppression(current_ward_df, 'first_level_suppression', 'Rank', 'second_level_suppression')
        logging.info("Current months Ward Level Inpatient DataFrame with second_level_suppression column:")
        logging.info(current_ward_df.head())    

        # Add field showing whether suppression is required at current level as a result of suppression at upper level
        current_ward_df = add_suppression_required_from_upper_level_column(current_site_df, current_ward_df,
                                                                          'upper_level_suppression_required', 'Site Code', 'site_level_suppression_required')    
        logging.info("Current months Ward Level Inpatient DataFrame with upper_level_suppression_required column:")
        logging.info(current_ward_df.head())
        ...
        current_ward_df = confirm_row_level_suppression(current_ward_df, 'ward_level_suppression_required', 'first_level_suppression',
                                                       'second_level_suppression', 'upper_level_suppression_required')
        logging.info("Current months Ward Level Inpatient DataFrame with ward_level_suppression_required column:")
        logging.info(current_ward_df.head())

        # Convert all fields that can contain suppression values from numeric (integer) to string to avoid a data type mismatch error when suppression is applied
        current_ward_df = convert_fields_to_object_type(current_ward_df, fields_for_converting)
        logging.info("Current months Ward Level Inpatient DataFrame with columns impacted by suppression converted to string:")
        logging.info(current_ward_df.head())
        ...
        current_ward_df = suppress_data(current_ward_df, 'ward_level_suppression_required', 'first_level_suppression')
        logging.info("Current months Ward Level Inpatient DataFrame with suppression applied:")
        logging.info(current_ward_df.head())      
        
        # Reorder current Ward level DataFrame by ICB Code, Trust Code, Site Code and Total Responses (Highest to Lowest)
        current_ward_df = sort_dataframe(current_ward_df, ['ICB Code', 'Trust Code', 'Site Code', 'Total Responses'], [True, True, True, False])
        logging.info("Current months Ward Level Inpatient DataFrame after sort:")
        logging.info(current_ward_df.head())           

        # Move responses for Independent Service Providers to bottom of DataFrame aligned with Stakeholder requirements for output
        current_ward_df = move_independent_provider_rows_to_bottom(current_ward_df)
        logging.info("Current months Ward Level Inpatient DataFrame with IS1 orgs moved to bottom:")
    ...
    # With Ward Name as a manual input field, replace special characters to avoid processing errors
        ...
        target_chars = ['>', '<', '/', '+', '*', '!', '£', '$', '"', '%', '^', '=', '#', '@']
        current_ward_df = replace_character_in_columns(current_ward_df, 'Ward Name', target_chars, '-')
        logging.info("Current months Ward Level Inpatient DataFrame with special characters in Ward name replaced:")
        logging.info(current_ward_df.head())        
    ...
    # Generate Macro Excel Backsheet Dropdown Lists to ensure all tab filters in the output contain correct ICB/Trust/Site/Ward name details
        ...
        bs_icb_detail_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['ICB Code', 'ICB Name']))
        ...
        bs_site_ward_detail_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['ICB Code', 'Trust Code', 'Trust Name', 
                                                                                                'Site Code', 'Site Name', 'Ward Name']))
        bs_site_ward_detail_df = sort_dataframe(bs_site_ward_detail_df, ['ICB Code', 'Trust Code', 'Trust Name', 
                                                                                                'Site Code', 'Site Name', 'Ward Name'], 
                                                [True, True, True, True, True, True])
        bs_site_ward_detail_df = move_independent_provider_rows_to_bottom(bs_site_ward_detail_df)
        ...
        bs_trust_code_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['Trust Code']))
        ...
        bs_trust_name_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['Trust Name']))
        ...
        bs_site_code_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['Site Code']))
        ...
        bs_site_name_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['Site Name']))
        ...
        bs_ward_name_df = remove_duplicate_rows(limit_retained_columns(current_ward_df, ['Ward Name']))
    ...
    # Remove unwanted columns and reorder retained columns for each DataFrame to ensure it conforms with stakeholder requirements of final outputs. 
        ...
        current_icb_df = remove_columns(current_icb_df, ['first_level_suppression', 'second_level_suppression', 'icb_level_suppression_required'])

        # Specify the correct column order for ICB level output
        icb_output_column_order = ['ICB Code', 'ICB Name', 'Total Responses', 'Total Eligible', 'Percentage Positive', 'Percentage Negative',
                                   'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 'Dont Know']
        ...
        fields_to_convert_to_object_type = ['Total Responses', 'Total Eligible', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_icb_df = convert_fields_to_object_type(current_icb_df, fields_to_convert_to_object_type)        
    
        # Clean DataFrame columns replacing with 'NA'
        current_icb_df = replace_missing_values(current_icb_df, 'NA')
        logging.info("Current months ICB Level Inpatient DataFrame following column clean, removal and reorder:")
        logging.info(current_icb_df.head()) 

        # Org Level - Remove columns not required in final Org level output including suppression helper columns and period 
        current_org_df = remove_columns(current_org_df, ['ICB Name', 'Rank', 'first_level_suppression', 'second_level_suppression', 
                                                         'upper_level_suppression_required', 'trust_level_suppression_required'])

        # Specify the correct column order for Org level output
        org_output_column_order = ['ICB Code', 'Trust Code', 'Trust Name', 'Total Responses', 'Total Eligible', 'Percentage Positive', 'Percentage Negative',
                                   'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 'Dont Know', 'Mode SMS', 'Mode Electronic Discharge',
                                   'Mode Electronic Home', 'Mode Paper Discharge', 'Mode Paper Home', 'Mode Telephone', 'Mode Online', 'Mode Other'] 
        ...
        fields_to_convert_to_object_type = ['Total Responses', 'Total Eligible', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_org_df = convert_fields_to_object_type(current_org_df, fields_to_convert_to_object_type)     

        # Clean DataFrame columns replacing with 'NA'
        current_org_df = replace_missing_values(current_org_df, 'NA')

        logging.info("Current months Org Level Inpatient DataFrame following column clean, removal and reorder:")
        logging.info(current_org_df.head()) 

        # Site Level - Remove columns not required in final Site level output including suppression helper columns and period 
        current_site_df = remove_columns(current_site_df, ['ICB Name', 'Rank', 'first_level_suppression', 'second_level_suppression', 
                                                         'upper_level_suppression_required', 'site_level_suppression_required'])

        # Specify the correct column order for Site level output
        site_output_column_order = ['ICB Code', 'Trust Code', 'Trust Name', 'Site Code', 'Site Name', 'Total Responses', 'Total Eligible', 'Percentage Positive', 
                                    'Percentage Negative', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 'Dont Know']
        ...
        fields_to_convert_to_object_type = ['Total Responses', 'Total Eligible', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_site_df = convert_fields_to_object_type(current_site_df, fields_to_convert_to_object_type)   

        # Clean DataFrame columns replacing with 'NA'
        current_site_df = replace_missing_values(current_site_df, 'NA')
        logging.info("Current months Site Level Inpatient DataFrame following column clean, removal and reorder:")
        logging.info(current_site_df.head()) 
        
        # Ward Level - Remove columns not required in final Ward level output including suppression helper columns and period 
        current_ward_df = remove_columns(current_ward_df, ['ICB Name', 'Response Rate', 'Rank', 'first_level_suppression', 'second_level_suppression', 
                                                         'upper_level_suppression_required', 'ward_level_suppression_required'])

        # Specify the correct column order for Ward level output
        ward_output_column_order = ['ICB Code', 'Trust Code', 'Trust Name', 'Site Code', 'Site Name', 'Ward Name', 'Total Responses', 'Total Eligible', 
                                    'Percentage Positive', 'Percentage Negative', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                    'Dont Know', 'First Speciality', 'Second Speciality']
        ...
        fields_to_convert_to_object_type = ['Total Responses', 'Total Eligible', 'Very Good', 'Good', 'Neither Good nor Poor', 'Poor', 'Very Poor', 
                                 'Dont Know', 'Percentage Positive', 'Percentage Negative']
        current_ward_df = convert_fields_to_object_type(current_ward_df, fields_to_convert_to_object_type)   

        # Clean DataFrame columns replacing with 'NA'
        current_ward_df = replace_missing_values(current_ward_df, 'NA')
        logging.info("Current months Ward Level Inpatient DataFrame following column clean, removal and reorder:")
        logging.info(current_ward_df.head()) 
        ...
        logging.info("DataFrames written to specified location of specified Workbook sheets.")
        

        # Ensure the Period subheadings on the Summary sheet are dynamically updated with correct current/previous period labels.
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=3, 
                                    data=current_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=4, 
                                    data=current_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=5, 
                                    data=current_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=6, 
                                    data=previous_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=7, 
                                    data=current_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=8, 
                                    data=previous_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=9, 
                                    data=current_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Summary', start_row=7, start_col=10, 
                                    data=previous_fft_period, font_size=10, bg_color = "BFBFBF", bold=True, font_name = "Verdana",
                                    align_horizontal = "center", align_vertical = "center")
        logging.info("Summary sheet subheadings updated to correctly reflect current/previous periods.")
        ...
        format_column_as_percentage(Inpatient_Excel_Workbook, sheet_name='ICB', start_row=15, start_cols=[5, 6], percentage_style=percentage_style)
        format_column_as_percentage(Inpatient_Excel_Workbook, sheet_name='Trusts', start_row=15, start_cols=[6, 7], percentage_style=percentage_style)
        format_column_as_percentage(Inpatient_Excel_Workbook, sheet_name='Sites', start_row=15, start_cols=[8, 9],percentage_style=percentage_style)
        format_column_as_percentage(Inpatient_Excel_Workbook, sheet_name='Wards', start_row=15, start_cols=[9, 10], percentage_style=percentage_style)
        logging.info("ICB, Trust, Site and Ward tabs updated to ensure percentage columns are displayed correctly.")

        # Define dynamic 'Note' sheet title. MAKE SURE THIS IS THE FINAL ACTION SO FILE SAVES AND OPENS ON 'Note' TAB.
        notepage_label = combine_text_and_dataframe_cells('Inpatient Friends and Family Test (FFT) Data -', current_fft_period)
        # Ensure the Workbook title on the 'Note' page updates dynamically with current FFT period suffix and correct format
        update_cell_with_formatting(Inpatient_Excel_Workbook, sheet_name='Notes', start_row=2, start_col=1, 
                                    data=notepage_label, font_size=18, bg_color = "FFFFFF", font_name = "Aptos Narrow",
        ...
        save_macro_excel_file(Inpatient_Excel_Workbook, template_file_path, output_file_path, new=True, prefix=prefix_name, fft_period_suffix=current_fft_period)
        
        logging.info("Current months Inpatient Excel output generated from processed DataFrames")
