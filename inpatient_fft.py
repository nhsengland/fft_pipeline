...
def main():
    ...
    try:
        ...
        logging.info("Current month FFT file:")
        logging.info( current_month_file)
        logging.info("Previous month FFT file:")
    ...
    #   - build 'England (including ISPs)' and 'England (excluding ISPs)' values
    #   - create aggregated data for rolling totals file and Summary data tab
    #   - aggregation to ICB level (required but not in the Extract file)
        ...
        logging.info("Organisation Level DataFrame preview:")
        logging.info(current_org_df.head())

        # Validate field lengths: Yearnumber (7), Org code (3 or 5), STP Code (3)
        ...
        # Validate numeric fields (int and float types)
        # Validate integer columns
        numeric_columns = [
            '1 Very Good', '2 Good', '3 Neither good nor poor', '4 Poor',
            '5 Very poor', '6 Dont Know', 'Total Responses', 'Total Eligible'
        ]
        current_org_df = validate_numeric_columns(current_org_df, numeric_columns, 'int')
        current_org_df = validate_numeric_columns(current_org_df, ['Prop Pos', 'Prop Neg'], 'float')
        logging.info("Data type validation complete.")

        # Get periodname and yearnumber for FFT_Period generation
        # Extract period information
        current_periodname = get_cell_content_as_string(
            current_org_df, source_row=0, source_col='Periodname'
        )
        current_yearnumber = get_cell_content_as_string(
            current_org_df, source_row=0, source_col='Yearnumber'
        )
        ...
        # Remove Period columns not needed for processing
        current_org_df = remove_columns(current_org_df, ['Period', 'Yearnumber', 'Periodname'])
        logging.info("DataFrame after column removal:")
        logging.info(current_org_df.head())
