...
def validate_column_length(df, columns, lengths):
    ...
    Parameters:
    d: DataFrame containing columns to check
...
def map_fft_period(periodname, yearnumber):
    ...
    Returns:
    - Tuple containing (fft_period_abbrev, fft_period_filename, fft_period_datetime) where:
      - fft_period_abbrev: The abbreviated period format (e.g., 'Jan-25') for display
      - fft_period_filename: The period format for filenames (e.g., 'january-2025')
      - fft_period_datetime: Datetime object representing the period (e.g., datetime(2025, 1, 1))
    """
    import datetime

    # Define dictionaries for month names and numbers
    month_abbrev = {
        "JANUARY": "Jan",
        "FEBRUARY": "Feb",
        "MARCH": "Mar",
    ...
    if periodname in ["JANUARY", "FEBRUARY", "MARCH"]:
        # Use the end year (e.g., JANUARY in 2024-25 is January 2025)
        year_to_use = int(start_year) + 1
        fft_period_abbrev = f"{month_abbrev[periodname]}-{end_year}"  # For display (e.g., Jan-25)
        fft_period_filename = f"{month_full[periodname]}-{year_to_use}"  # For filename (e.g., january-2025)
    else:
        # Use the start year (e.g., APRIL in 2024-25 is April 2024)
        year_to_use = int(start_year)
        fft_period_abbrev = f"{month_abbrev[periodname]}-{start_year[2:]}"  # For display (e.g., Apr-24)
        fft_period_filename = f"{month_full[periodname]}-{year_to_use}"  # For filename (e.g., april-2024)

    # Create a datetime object for the first day of the month
    fft_period_datetime = datetime.datetime(year_to_use, month_number[periodname], 1)
...
def create_percentage_field(
    ...
    total_column: total response field used to establish the percentage the two other columns represent.
...
def suppress_data(df, overall_suppression_field, first_level_suppression_field):
    ...
    response_column_suppression = [
        ...
        "Very Poor",
        "Dont Know",
        "Don't Know", # Added with apostrophe to handle both versions
    ]
...
def standardise_icb_names(df, icb_name_column="ICB Name"):
    """
    Function to standardise ICB (Integrated Care Board) names in a DataFrame.
    Converts full NHS organization names to shorter format.

    For example:
    - "NHS LANCASHIRE AND SOUTH CUMBRIA INTEGRATED CARE BOARD" becomes "LANCASHIRE AND SOUTH CUMBRIA ICB"

    Parameters:
    df: DataFrame containing ICB names to standardize
    icb_name_column: Name of the column containing ICB names (default: "ICB Name")

    Returns:
    - df: DataFrame with standardised ICB names
    """
    # Check if the specified column exists in the DataFrame
    if icb_name_column not in df.columns:
        logging.warning(f"Column '{icb_name_column}' not found in DataFrame. Returning DataFrame unchanged.")
        return df

    # Function to standardize a single ICB name
    def standardize_name(name):
        if not isinstance(name, str):
            return name

        # Remove "NHS " prefix if it exists
        if name.startswith("NHS "):
            name = name[4:]

        # Replace "INTEGRATED CARE BOARD" with "ICB"
        name = name.replace("INTEGRATED CARE BOARD", "ICB")
    ...
    # Apply the standardization function to the ICB name column
    df[icb_name_column] = df[icb_name_column].apply(standardize_name)
...
def save_macro_excel_file(
    ...
    fft_period_suffix: A tuple of format strings for the period. This comes from map_fft_period function
                      and contains (fft_period_abbrev, fft_period_filename, fft_period_datetime).
                      For display it uses fft_period_abbrev (e.g. 'Aug-25'),
                      but for filename it uses fft_period_filename (e.g. 'august-2025').

    Returns:
    - None: Excel file saved to specified file path.
    """
    if new:
        # Get the period format for filename (second element in the tuple)
        period_for_filename = fft_period_suffix[1] if isinstance(fft_period_suffix, tuple) else fft_period_suffix

        # Update prefix from "FFT-inpatient-data" to "friends-and-family-test-inpatient-data"
        new_prefix = "friends-and-family-test-inpatient-data" if prefix == "FFT-inpatient-data" else prefix

        # Construct the new file name
        new_file_name = f"{new_prefix}-{period_for_filename}.xlsm"
