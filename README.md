...
The Friends and Family Test (FFT) is the UK's largest patient feedback program, collecting ~2 million responses monthly. This pipeline transforms raw monthly FFT Excel data into formatted, suppression-compliant reports published by NHS England.
...
- Any organization with 1-4 responses gets suppressed (replaced with `*`)
...
- Maintains organizational hierarchy throughout
...
fft_pipeline/
├── src/fft/
│   ├── config.py       # Centralized configuration (paths, mappings, constants)
...
3. **Standardize**: Rename columns to common schema
...
2. **Second-level**: The next-lowest responding organization also gets suppressed (prevents "Total - Known = Suppressed" calculation)
...
└── FFT_Ambulance_V1 Aug-25.xlsx

data/inputs/rolling_totals/
├── rolling_totals_inpatient.csv
├── rolling_totals_ae.csv
└── rolling_totals_ambulance.csv

data/inputs/templates/
├── FFT-inpatient-data-template.xlsm
├── FFT-ae-data-template.xlsm
└── FFT-ambulance-data-template.xlsm
